...
//	HAL_UART_Receive_DMA(&huart2, (uint8_t *)&u8Uart2Data, 1);
...
//	uint8_t validNum = 0;;
//	for(i=0;i<7;i++){
//		if(u8Uart2Data[i] == 0xfa){
//			validNum = 1;
//		}
//		if(validNum){
//			onParseProtocol(u8Uart2Data[i]);
//		}
//	}
//	onParseProtocol(u8Uart2Data[0]);
//	static uint32_t cnt =0;
//	cnt ++;
//	HAL_UART_Receive_IT(&huart2, (uint8_t *)&u8Uart2RxData, 1);
}

void AppUart2ReceiveData(uint8_t *pData, uint32_t size)
{
	HAL_UART_Receive(&huart2, pData, size, 0xFFFFFFFF);
}
